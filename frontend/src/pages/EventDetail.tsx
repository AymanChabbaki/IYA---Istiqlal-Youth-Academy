          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <DialogDescription className="text-sm mt-0.5">
                Create your Istiqlal Youth Academy account and register for{' '}
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-2">
          ...
          <Alert className="border-primary/30 bg-primary/5">
            ...
            <AlertDescription className="text-sm">
              By registering, you agree to create an Istiqlal Youth Academy account. You can log in later using your email and password.
            </AlertDescription>
          </Alert>
...
const EventDetail = () => {
  ...
  const generateDefaultBadge = () => {
    setTimeout(() => {
      try {
        ...
        doc.text('Istiqlal Youth Academy', 105, 25, { align: 'center' });
  ...
  const finalizePDF = (doc: jsPDF, token: string, eventId: string, ch: number) => {
    ...
    doc.text('Email: contact@istiqlalyouthacademy.org', 105, fy + 11, { align: 'center' });
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mb-1.5">
                          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-primary/5 text-primary border border-primary/10">
                            ...
                            <span className="text-xs font-bold tabular-nums text-primary/90">
                              {format(new Date(session.startAt), 'E, MMM dd | HH:mm')} to {format(new Date(session.endAt), 'HH:mm')}
                            </span>
                          </div>
                          {session.location && (
              ...
              {!isAuthenticated && event.allowGuestRegistration && (
                <motion.div key="visitor-banner" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <p className="text-sm text-emerald-800 dark:text-emerald-300">
                    <strong>No account needed!</strong> You can register as a visitor and we'll create your Istiqlal Youth Academy account automatically.
                  </p>
                </motion.div>
              )}
              {!isAuthenticated && !event.allowGuestRegistration && (
