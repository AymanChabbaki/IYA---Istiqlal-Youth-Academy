...
export const generate = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const imagePromptText =
      ...
      `for example an oversized microphone, a tiny scales-of-justice mascot at her side, ` +
      `a briefcase full of policy papers, stacks of books, etc. ` +
      // Environment
      `Setting: an elegant Moroccan civic hall in 2030, decorated for a leadership summit, ` +
      `warm lights mixing Moroccan zellige patterns with elegant civic/political motifs, ` +
      `a banner or badge visible in the scene that reads "Istiqlal Youth Academy" and "Women's Day 2026". ` +
      // Her appearance
      (hasPhoto
    ...
    const complimentPrompt =
      ...
      `عضوة نشيطة في الأكاديمية الاستقلالية للشباب بالمغرب. ` +
