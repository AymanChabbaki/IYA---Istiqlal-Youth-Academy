...
export const updateUserRole = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { role, staffRole, region, committeeId } = req.body;
  ...
  if (committeeId !== undefined) updateData.committeeId = committeeId || null;
...
export const updateUser = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (region !== undefined) updateData.region = region || null;
  if (iqlim !== undefined) updateData.iqlim = iqlim;
