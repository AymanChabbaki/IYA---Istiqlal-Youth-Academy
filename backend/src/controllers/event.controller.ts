...
export const approveRegistration = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const emailTemplate = emailTemplates.registrationApproved(
    ...
    comment,
    {
      ...
      badgeDownloadUrl,
      frontendUrl: process.env.FRONTEND_URL || 'https://istiqlalyouthacademy.vercel.app',
...
async function getBadgeTemplate(): Promise<Buffer | null> {
  ...
  const frontendUrl = process.env.FRONTEND_URL || 'https://istiqlalyouthacademy.vercel.app';
  ...
  return new Promise<Buffer>(async (resolve, reject) => {
    ...
    if (ev.useCustomBadge) {
      ...
      doc.fillColor('#ffffff').fontSize(28).font('Helvetica-Bold')
        .text('Istiqlal Youth Academy', 0, 38, { align: 'center', width: pageW });
      ...
      doc.font('Helvetica').fontSize(8).text('Email: contact@istiqlalyouthacademy.org', 0, footerY + 24, { align: 'center', width: pageW });
    }
...
export const registerAsGuest = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await prisma.notification.create({
    data: {
      ...
      content: `Welcome to Istiqlal Youth Academy! Your registration for "${event.title}" is pending staff approval.`
    }
  });
